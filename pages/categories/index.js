...
// import { div, div, Carousel } from "antd";
...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const debouncedHandleScroll = debounce(handleScroll, 4000);
  ...
  useEffect(() => {
    axios
      .get("https://api.sedihisham.com/categories/findall")
      .then((response) => {
  ...
  useEffect(() => {
    const getMainImage = () => {
      axios
        .get("https://api.sedihisham.com/pages/getall/products")
        .then((res) => {
          const image = res.data.find(
            (item) => item.categoryImage === "products_page"
          )?.path_image;
          setMainImage(image || "");
        });
    };
  ...
  useEffect(() => {
    if (categories.length > 0) {
      const animateCategory = (div, props) => {
        gsap.to(div, {
          ...props,
          duration: 1,
          scale: 0.89,
          scrollTrigger: {
            ...
            // once: true,
            // markers: true,
            // onLeaveBack: () => gsap.set(div, {clearProps: "all"})
          },
        });
      };

      categories.forEach((category, index) => {
        const categoryDiv = document.querySelectorAll(".categorydiv")[index];

        animateCategory(categoryDiv.querySelector(".grots"), {
          scale: 0.9,
          rotation: 0,
          duration: 1,
          opacity: 1,
          ...
          scrollTrigger: {
            ...
            // markers: true,
            onLeaveBack: () => gsap.set(categoryDiv, { clearProps: "all" }),
          },
        });
      });
  ...
  return (
    <div>
      <div className={` products w-[80%] mx-[10%]`}>
        <SEO 
      ...
      />
        <div className={`w-full h-full  categoryMainImage`}>
          {!isMobile ? (
            <img alt="صورة سيدي هشام" 
              style={{ objectFit: "contain" }}
              width={120}
              height={100}
              src={"https://api.sedihisham.com/" + mainImage}
            />
          ) : (
            <img alt="صورة سيدي هشام" 
              style={{ objectFit: "contain" }}
              width={isMobile ? "120vw" : "80px"}
              height="40"
              src={"https://api.sedihisham.com/" + mainImage}
            />
          )}
          ...
          <div lg={20} span={22}>
            <div className={`${styles.productsCon} productsCon`}>
              ...
              {categories.map((category, index) =>
                index % 2 != 0 ? (
                  <div className=" categorydiv flex md:flex-div flex-div justify-between">
                    <div
                    ...
                    >
                      ...
                      >
                        <div
                          className={`${styles.categoryInfo} space-x-2  categoryInfo`}
                        >
                          <div className="pr-2 order-1">
                            <img
                              width={isMobile ? "120vw" : "80"}
                              height={isMobile ? "120vw" : "80"}
                              className={`${styles.categoryIcon} categoryIcon md:mt-2 md:ml-2`}
                              src={`https://api.sedihisham.com/${category.icon_url}`}
                              alt="No image"
                            />
                          </div>
                          <div>
                            <div
                              className={`${styles.categoryName} text-[80px] font-arabicMedium categoryName`}
                            >
                              {parse(
                                category !== undefined
                            ...
                            <div
                              className={`${styles.categoryProducts} font-arabicLight categoryProducts`}
                            >
                              {/* {category.products.map((child, index) => (
                                <>
                                  {child.name}
                            ...
                            <div
                              onClick={() =>
                                router.push(
                                  `/categories/product-content/${category.id}`
                                )
                              }
                              className={`${styles.seeMore} font-arabicMedium seeMore`}
                            >
...
// Debounce function to reduce the frequency of function calls
