  ...
  return (
    <div
      ...
      className={`pointer-events-auto relative mx-auto grid transform-gpu items-center border bg-[#7C1422] transition-all duration-500 ease-[cubic-bezier(0.22,1,0.36,1)] ${
        isStatic
          ? `
            h-[94px] w-full max-w-none
            grid-cols-[auto_minmax(0,1fr)_118px_minmax(0,1fr)_auto]
            gap-2 rounded-none
          ...
          : `
            mt-3 h-[68px]
            w-[calc(100%-24px)] max-w-[1380px]
            grid-cols-[auto_minmax(0,1fr)_84px_minmax(0,1fr)_auto]
            gap-1.5 rounded-full
            border-white/15
            px-3
            shadow-[0_18px_55px_rgba(0,0,0,0.3)]
            backdrop-blur-xl
            lg:grid-cols-[auto_minmax(0,1fr)_96px_minmax(0,1fr)_auto]
          `
      }`}
    >
      ...
      <Link
        ...
        className="
          group absolute left-1/2 top-1/2 z-30
...
export default function NavBar() {
  ...
  return (
    <>
      {/* الهيدر الأساسي: كامل عرض الصفحة */}
      ...
      {/* الهيدر الثابت: كومباكت وفي المنتصف */}
      <header
